...
3. 你会看到一个使用帮助弹窗，选择右下角的“OK”或者选择左下角的“设置”打开设置界面：
   ...
   - “多行命令模式(实验)”，默认不勾选。勾选后，单次命令模式下会多出一个灰色加号，点击后会进入多行命令输入模式，这时点击回车键将换行而非直接执行命令。
...
4. 点击 ShizukuRunner 界面上的“空/空”输入框或灰色加号：
   ...
   - **注意：**
     ...
     - 可用命令示例：`whoami`（查看当前用户）、`pm list packages -3 | head -5`（列出已安装的前五位第三方软件包名）、`wm size`（查看屏幕分辨率）等。
