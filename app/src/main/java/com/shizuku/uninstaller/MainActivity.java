...
public class MainActivity extends Activity {
    ...
    // 供外部调用的静态方法：获取多行模式设置
    public static boolean isMultiLineModeEnabled() {
        if (instance == null || instance.sp == null) return false;
        return instance.sp.getBoolean("multi_line_mode", false);
    }

    // 供外部调用的静态方法：应用输入模式到 EditText（合并自动换行和多行模式）
    public static void applyInputModeToEditText(EditText editText) {
    ...
    public void ex(View view) {
        //本函数绑定了猫猫头像的点击事件，让list变不可见，让EditText可见。
